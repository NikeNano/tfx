...
# Directory that optional stamped model output is stored in.
_STAMPED_MODEL_DIR = 'stamped_model'
_ASSETS_EXTRA = 'assets.extra'
_WARMUP_FILENAME = 'tf_serving_warmup_requests'
...
class Executor(base_executor.BaseExecutor):
  ...
  def _CreateWarmupModel(self, blessing: types.Artifact, model_path: str,
    ...
    warmup_model_path = os.path.join(blessing.uri, _STAMPED_MODEL_DIR)
    io_utils.copy_dir(src=model_path, dst=warmup_model_path)
    if warmup_requests:
      io_utils.write_tfrecord_file(
          os.path.join(warmup_model_path, _ASSETS_EXTRA, _WARMUP_FILENAME),
